...
class GNNClassifier(nn.Module):
    def __init__(self, num_classes=3):  # 默认三分类
        super().__init__()
        # 保持GCN层名称不变
        ...
        self.fc = nn.Linear(16, num_classes)  # 多分类输出
...
def run_multiclass_cv_once(all_raw_matrices, labels, n_splits=5, epochs_per_fold=500):
    """Run one multi-class CV experiment and return mean/std accuracy."""
...
def run_permutation_test_for_copula_3class(
        selected_dataset,
        original_labels,
        n_splits=5,
        epochs_per_fold=500,
    ...
    """Permutation test for Copula 3-class task."""
    print("\n=== Copula 3-Class Permutation Test ===")
    print("Task: 3-class (AD vs HC vs FTD)")
    print(f"Samples: {len(original_labels)} | Splits: {n_splits} | Epochs/Fold: {epochs_per_fold}")
    print(f"Permutations: {n_permutations}")

    # Real-label score
    real_mean_acc, real_std_acc, real_fold_acc = run_multiclass_cv_once(
        selected_dataset, original_labels, n_splits=n_splits, epochs_per_fold=epochs_per_fold
    )

    # Permutation scores
    rng = np.random.default_rng(random_seed)
    perm_scores = []
    labels_np = original_labels.cpu().numpy().copy()
    ...
    for i in range(n_permutations):
        ...
        perm_mean_acc, _, _ = run_multiclass_cv_once(
            selected_dataset, shuffled_tensor, n_splits=n_splits, epochs_per_fold=epochs_per_fold
        )
    ...
    out_path = os.path.join(os.path.dirname(__file__), "copula_3class_permutation_result.json")
...
# Only run Copula 3-class permutation testing.
if TORCH_GEOMETRIC_AVAILABLE:
    ...
    run_permutation_test_for_copula_3class(
