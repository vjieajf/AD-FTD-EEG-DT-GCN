...
if TORCH_GEOMETRIC_AVAILABLE:
    ...
    binary_tasks = [(0, 1), (0, 2), (1, 2)]
    all_results = []
    
    for class_a, class_b in binary_tasks:
        print(f"\n--- 二分类任务: 类别{class_a} vs 类别{class_b} ---")
        
        # 创建二分类数据集
        binary_matrices, binary_labels = create_binary_dataset(selected_dataset, labels, class_a, class_b)
        
        # 运行动态阈值和固定阈值的对比实验
        dynamic_results, fixed_results = cross_validation_ablation(
            binary_matrices, binary_labels, n_splits=5, epochs_per_fold=500
        )
