...
def run_kfold(raw_data, labels, band_name="unknown"):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(raw_data, labels), start=1):
        ...
        model = MultiBandGNN(in_dim=N_CHANNELS + 2, num_classes=N_CLASSES).to(device)
...
def save_results(all_results, cm_stack, case_order):
    ...
    cm_path = os.path.join(OUTPUT_DIR, "multiband_3class_confusion_matrix_4datasets_20cases.npy")
    np.save(cm_path, cm_stack)

    json_path = os.path.join(OUTPUT_DIR, "multiband_3class_results_4datasets_20cases.json")
    payload = {
...
def main():
    ...
    print("开始 4 个多频段 FC 数据集的三分类训练（固定标签顺序）...")
    ...
    for dataset_name, dataset_path in DATASET_FILES.items():
        ...
        all_case_results[dataset_name] = {}

        for band_name in BAND_NAMES:
            case_name = f"{dataset_name}/{band_name}"
            print(f"\n--- Case: {case_name} ---")
            band_results = run_kfold(band_data_dict[band_name], LABELS, band_name=case_name)

            print(f"[{case_name}] Accuracy: {band_results['mean_acc']:.4f} ± {band_results['std_acc']:.4f}")
            print(f"[{case_name}] F1-macro: {band_results['mean_f1_macro']:.4f} ± {band_results['std_f1_macro']:.4f}")
            print(f"[{case_name}] Confusion Matrix:")
            print(band_results["confusion_matrix"])

            all_case_results[dataset_name][band_name] = {
    ...
    cm_stack = np.stack(cm_list, axis=0)  # (20, 3, 3)

    print("\n=== 四数据集 × 五频段三分类结果汇总 ===")
    for dataset_name in DATASET_FILES.keys():
        for band_name in BAND_NAMES:
            r = all_case_results[dataset_name][band_name]
            print(
                f"{dataset_name:<8} | {band_name:<6} | "
