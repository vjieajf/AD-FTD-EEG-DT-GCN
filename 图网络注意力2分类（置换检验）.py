...
torch.cuda.manual_seed_all(RANDOM_SEED)  # 对于多GPU情况
...
class GNNClassifier(nn.Module):
    def __init__(self, num_classes=2):  # 修改为二分类
        super().__init__()
        # 保持GCN层名称不变
...
def adaptive_threshold_training(model, optimizer, train_matrices, train_labels,
                                val_matrices, val_labels, epochs=100, device='cpu', num_classes=2):
    ...
    # 预先处理验证集数据，使用固定阈值
    val_dataset_fixed = dynamic_threshold_preprocessing(val_matrices, current_threshold * 100)  # 使用原始函数
    ...
    for epoch in range(epochs):
        ...
        if current_threshold not in train_feature_cache:
            train_feature_cache[current_threshold] = dynamic_threshold_preprocessing(  # 使用原始函数
        ...
        for data in train_loader:
            ...
            # 重构损失应基于当前图的边结构而不是节点特征
            # 使用当前图的边索引来重建邻接矩阵用于重构损失
            adj_reconstructed = torch.zeros_like(original_adj)
            if data.edge_index.size(1) > 0 and data.edge_index.size(0) >= 2:
                # 确保索引不超出邻接矩阵的范围
                valid_indices = (data.edge_index[0] < adj_reconstructed.size(0)) & (data.edge_index[1] < adj_reconstructed.size(1))
                valid_edges = data.edge_index[:, valid_indices]
                if valid_edges.size(1) > 0:
                    adj_reconstructed[valid_edges[0], valid_edges[1]] = 1.0
            
            # 使用均方误差作为重构损失
            recon_loss = F.mse_loss(adj_reconstructed, original_adj)
        ...
        def scale_epoch_loss(loss_val):
            divisor = 1.0
            temp_val = abs(loss_val)
            count = 0
            while temp_val >= 10:
                temp_val /= 10
                count += 1
            ...
            return loss_val / divisor
        
        total_loss_scaled = scale_epoch_loss(total_loss)
        cls_loss_total_scaled = scale_epoch_loss(cls_loss_total)
        recon_loss_total_scaled = scale_epoch_loss(recon_loss_total)
        cont_loss_total_scaled = scale_epoch_loss(cont_loss_total)
        
        loss_history.append(total_loss_scaled)
        
        if prev_total_loss is None:
            loss_change = 0.01
        else:
            loss_change = prev_total_loss - total_loss_scaled

        prev_total_loss = total_loss_scaled
        
        total_loss = total_loss_scaled
        cls_loss_total = cls_loss_total_scaled
        recon_loss_total = recon_loss_total_scaled
        cont_loss_total = cont_loss_total_scaled
        ...
        if (epoch + 1) % 20 == 0 or epoch == epochs - 1:
            def format_loss_value(loss_val):
                divisor = 1.0
                temp_val = abs(loss_val)
                count = 0
                while temp_val >= 100:
            ...
            total_loss_formatted = format_loss_value(total_loss)
            cls_loss_formatted = format_loss_value(cls_loss_total)
            recon_loss_formatted = format_loss_value(recon_loss_total)
            cont_loss_formatted = format_loss_value(cont_loss_total)
            ...
            if len(all_targets) > 0:
                ...
                print(f"Epoch {epoch + 1}/{epochs}: Threshold={current_threshold:.3f} | "
                      f"Loss={total_loss_formatted:.4f} (Cls:{cls_loss_formatted:.4f}, Recon:{recon_loss_formatted:.4f}, Cont:{cont_loss_formatted:.4f}) | "
...
def fixed_threshold_training(model, optimizer, train_matrices, train_labels,
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            output = model(data)
            loss = F.cross_entropy(output, data.y)  # 简化为只使用分类损失
    ...
    return model, 30.0, best_val_acc, best_se, best_sp, best_p
...
def cross_validation_ablation(all_raw_matrices, labels, n_splits=5, epochs_per_fold=100):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(all_raw_matrices)):
        ...
        # 动态阈值训练
        _, best_threshold, best_val_acc_dynamic, best_se, best_sp, best_p = adaptive_threshold_training(
        ...
        # 固定阈值训练
        _, _, best_val_acc_fixed, best_se_fixed, best_sp_fixed, best_p_fixed = fixed_threshold_training(
    ...
    print(f"动态阈值 - 灵敏度 (Se): {compute_overall_metric(dynamic_results['se'])}")
    print(f"动态阈值 - 特异性 (Sp): {compute_overall_metric(dynamic_results['sp'])}")
    print(f"动态阈值 - 精确度 (P): {compute_overall_metric(dynamic_results['p'])}")
    
    print(f"固定阈值 - 灵敏度 (Se): {compute_overall_metric(fixed_results['se'])}")
    print(f"固定阈值 - 特异性 (Sp): {compute_overall_metric(fixed_results['sp'])}")
    print(f"固定阈值 - 精确度 (P): {compute_overall_metric(fixed_results['p'])}")
    
    return dynamic_results, fixed_results


def run_binary_cv_once(all_raw_matrices, labels, n_splits=5, epochs_per_fold=500):
    """Run one binary CV experiment and return mean/std accuracy."""
    fold_best_val_acc, _, _, _, _ = cross_validation(
        all_raw_matrices, labels, n_splits=n_splits, epochs_per_fold=epochs_per_fold
    )
    return float(np.mean(fold_best_val_acc)), float(np.std(fold_best_val_acc)), fold_best_val_acc


def bootstrap_mean_ci(values, n_bootstrap=2000, confidence=0.95, random_seed=RANDOM_SEED):
    """Compute bootstrap mean and percentile confidence interval."""
...
def run_permutation_test_for_copula_binary(
        selected_dataset,
        original_labels,
        class_a=0,
        class_b=1,
    ...
    """Permutation test for one Copula binary task."""
    # Build binary dataset first
    binary_matrices, binary_labels = create_binary_dataset(selected_dataset, original_labels, class_a, class_b)

    print("\n=== Copula Binary Permutation Test ===")
    print(f"Task: class {class_a} vs class {class_b}")
    print(f"Samples: {len(binary_labels)} | Splits: {n_splits} | Epochs/Fold: {epochs_per_fold}")
    print(f"Permutations: {n_permutations}")

    # Real-label score
    real_mean_acc, real_std_acc, real_fold_acc = run_binary_cv_once(
        binary_matrices, binary_labels, n_splits=n_splits, epochs_per_fold=epochs_per_fold
    )

    # Permutation scores
    rng = np.random.default_rng(random_seed)
    perm_scores = []
    labels_np = binary_labels.cpu().numpy().copy()

    for i in range(n_permutations):
        shuffled = labels_np.copy()
        rng.shuffle(shuffled)
        shuffled_tensor = torch.tensor(shuffled, dtype=torch.long)

        perm_mean_acc, _, _ = run_binary_cv_once(
            binary_matrices, shuffled_tensor, n_splits=n_splits, epochs_per_fold=epochs_per_fold
        )
        perm_scores.append(perm_mean_acc)

        if (i + 1) % 20 == 0 or i == n_permutations - 1:
            print(f"Permutation {i + 1}/{n_permutations}: mean Acc = {perm_mean_acc:.4f}")

    perm_scores = np.array(perm_scores, dtype=float)

    # 95% CI for the observed mean accuracy and the permutation null mean
    real_mean_ci, real_acc_ci = bootstrap_mean_ci(real_fold_acc, n_bootstrap=2000, confidence=0.95, random_seed=random_seed)
    perm_mean_ci, perm_acc_ci = bootstrap_mean_ci(perm_scores, n_bootstrap=2000, confidence=0.95, random_seed=random_seed + 1)

    # One-sided p-value: higher-is-better metric
    p_value = (1.0 + np.sum(perm_scores >= real_mean_acc)) / (1.0 + n_permutations)

    result = {
        "dataset": "copula",
        "task": f"{class_a}vs{class_b}",
        "n_splits": int(n_splits),
        "epochs_per_fold": int(epochs_per_fold),
        "n_permutations": int(n_permutations),
        "real_mean_acc": real_mean_acc,
        "real_std_acc": real_std_acc,
        "real_acc_ci_95": [float(real_acc_ci[0]), float(real_acc_ci[1])],
        "real_fold_acc": [float(x) for x in real_fold_acc],
        "perm_mean_acc": float(np.mean(perm_scores)),
        "perm_std_acc": float(np.std(perm_scores)),
        "perm_acc_ci_95": [float(perm_acc_ci[0]), float(perm_acc_ci[1])],
        "p_value": float(p_value)
    }

    print("\n=== Permutation Test Summary ===")
    print(f"Real labels: Acc = {real_mean_acc:.4f} +/- {real_std_acc:.4f}, 95% CI [{real_acc_ci[0]:.4f}, {real_acc_ci[1]:.4f}]")
    print(f"Permutation null: mean = {np.mean(perm_scores):.4f}, std = {np.std(perm_scores):.4f}, 95% CI [{perm_acc_ci[0]:.4f}, {perm_acc_ci[1]:.4f}]")
    print(f"One-sided p-value = {p_value:.6f}")

    out_path = os.path.join(os.path.dirname(__file__), "copula_binary_permutation_result.json")
    ...
    # 绘制置换检验的分布直方图
    plt.hist(perm_scores, bins=30, color='gray', alpha=0.7, label='Permuted Accuracies (Null Distribution)')
    # 绘制真实准确率的竖线
    plt.axvline(x=real_mean_acc, color='red', linestyle='--', linewidth=2, label=f'Real Accuracy ({real_mean_acc:.2f})')

    plt.title(f'Permutation Test Result: {class_a} vs {class_b}')
    ...
    # 保存图像
    plot_path = os.path.join(os.path.dirname(__file__), f"permutation_test_{class_a}_vs_{class_b}.pdf")
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    print(f"Permutation test plot saved to: {plot_path}")
    plt.show()
...
# Only run Copula binary permutation testing.
if TORCH_GEOMETRIC_AVAILABLE:
    ...
    # 置换次数可按算力调整，审稿常用>=200；更稳可设500/1000
    N_PERMUTATIONS = 200

    run_permutation_test_for_copula_binary(
        selected_dataset=selected_dataset,
        original_labels=labels,
        class_a=TASK_CLASS_A,
        class_b=TASK_CLASS_B,
        n_splits=5,
        epochs_per_fold=500,
    ...
    print("PyTorch Geometric is not available; permutation test was not executed.")
