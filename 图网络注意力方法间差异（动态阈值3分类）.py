...
def _paired_bootstrap_ci(diff, n_boot=5000, alpha=0.05, random_state=42):
...
def _holm_correction(pvals_dict):
...
def paired_significance_report(dynamic_results, fixed_results):
    # Fold-level macro metrics (per-fold mean across classes)
    metric_pairs = {
        "accuracy": (
            np.asarray(dynamic_results['acc'], dtype=float),
            np.asarray(fixed_results['acc'], dtype=float)
        ),
        "se_macro": (
            np.asarray([np.mean(x) for x in dynamic_results['se']], dtype=float),
            np.asarray([np.mean(x) for x in fixed_results['se']], dtype=float)
        ),
        "sp_macro": (
            np.asarray([np.mean(x) for x in dynamic_results['sp']], dtype=float),
            np.asarray([np.mean(x) for x in fixed_results['sp']], dtype=float)
        ),
        "precision_macro": (
            np.asarray([np.mean(x) for x in dynamic_results['p']], dtype=float),
            np.asarray([np.mean(x) for x in fixed_results['p']], dtype=float)
        )
    }

    raw_wilcoxon_p = {}
    report = {}
    for name, (dyn, fix) in metric_pairs.items():
        diff = dyn - fix
        n = int(diff.size)
        mean_diff = float(np.mean(diff))
        std_diff = float(np.std(diff, ddof=1)) if n > 1 else 0.0

        t_stat, t_p = ttest_rel(dyn, fix)
        t_stat = float(t_stat) if np.isfinite(t_stat) else float('nan')
        t_p = float(t_p) if np.isfinite(t_p) else float('nan')

        if np.allclose(diff, 0.0):
            w_stat, w_p = 0.0, 1.0
            z_approx = 0.0
        else:
            w_stat, w_p = wilcoxon(dyn, fix, zero_method='wilcox', correction=False, alternative='two-sided', mode='auto')
            # Normal approximation for effect size r
            mu_w = n * (n + 1) / 4
            sigma_w = np.sqrt(n * (n + 1) * (2 * n + 1) / 24)
            z_approx = (w_stat - mu_w) / sigma_w if sigma_w > 0 else 0.0

        raw_wilcoxon_p[name] = float(w_p)

        cohen_dz = float(mean_diff / std_diff) if std_diff > 0 else 0.0
        effect_r = float(abs(z_approx) / np.sqrt(n)) if n > 0 else 0.0

        report[name] = {
            "dynamic_mean": float(np.mean(dyn)),
            "fixed_mean": float(np.mean(fix)),
            "mean_diff_dynamic_minus_fixed": mean_diff,
            "bootstrap95ci_diff": _paired_bootstrap_ci(diff),
            "paired_t": {"t": t_stat, "p": t_p},
            "wilcoxon": {"W": float(w_stat), "p": float(w_p)},
            "effect_size": {"cohen_dz": cohen_dz, "r_from_wilcoxon_z": effect_r}
        }

    holm = _holm_correction(raw_wilcoxon_p)
    for name, p_adj in holm.items():
        report[name]["wilcoxon"]["p_holm"] = float(p_adj)
        report[name]["significant_after_holm_alpha_0_05"] = bool(p_adj < 0.05)
...
if TORCH_GEOMETRIC_AVAILABLE:
    ...
    stats_report = paired_significance_report(dynamic_results, fixed_results)

    # 仅输出检验结果
    print(json.dumps(stats_report, indent=2, ensure_ascii=False))

    with open("significance_test_dynamic_vs_fixed_3class.json", "w", encoding="utf-8") as f:
